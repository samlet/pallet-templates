...
decl_storage! {
	trait Store for Module<T: Trait> as {{cookiecutter.type_name}} {
		{{cookiecutter.type_name}} get(fn simple_map): map hasher(blake2_128_concat) T::AccountId => u32;
	}
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn set_single_entry(origin, entry: u32) -> DispatchResult {
			...
			<{{cookiecutter.type_name}}<T>>::insert(&user, entry);
		...
		fn get_single_entry(origin, account: T::AccountId) -> DispatchResult {
			...
			ensure!(<{{cookiecutter.type_name}}<T>>::contains_key(&account), Error::<T>::NoValueStored);
			let entry = <{{cookiecutter.type_name}}<T>>::get(account);
		...
		fn take_single_entry(origin) -> DispatchResult {
			...
			ensure!(<{{cookiecutter.type_name}}<T>>::contains_key(&user), Error::<T>::NoValueStored);
			let entry = <{{cookiecutter.type_name}}<T>>::take(&user);
		...
		fn increase_single_entry(origin, add_this_val: u32) -> DispatchResult {
			...
			let original_value = <{{cookiecutter.type_name}}<T>>::get(&user);

			let new_value = original_value.checked_add(add_this_val).ok_or(Error::<T>::MaxValueReached)?;
			<{{cookiecutter.type_name}}<T>>::insert(&user, new_value);
