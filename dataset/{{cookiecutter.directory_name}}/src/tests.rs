...
fn add_member_works() {
	ExtBuilder::build().execute_with(|| {
		assert_ok!({{cookiecutter.type_name}}::add_member(Origin::signed(1)));
...
fn cant_add_duplicate_members() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_noop!(
			{{cookiecutter.type_name}}::add_member(Origin::signed(1)),
...
fn cant_exceed_max_members() {
	ExtBuilder::build().execute_with(|| {
		...
		for i in 0..16 {
			assert_ok!({{cookiecutter.type_name}}::add_member(Origin::signed(i)));
		}

		// Try to add the 17th member exceeding the max
		assert_noop!(
			{{cookiecutter.type_name}}::add_member(Origin::signed(16)),
...
fn remove_member_works() {
	ExtBuilder::build().execute_with(|| {
		assert_ok!({{cookiecutter.type_name}}::add_member(Origin::signed(1)));
		assert_ok!({{cookiecutter.type_name}}::remove_member(Origin::signed(1)));
...
fn remove_member_handles_errors() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_noop!(
			{{cookiecutter.type_name}}::remove_member(Origin::signed(2)),
